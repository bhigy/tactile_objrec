...
bool MotorThread::getArmOptions(Bottle &b, const int &arm)
{
    ...
    if (Bottle *pB=b.find("play_start_pos").asList())
    {
        playPianoStartPos[arm].resize(pB->size());

        for (int i=0; i<pB->size(); i++)
            playPianoStartPos[arm][i]=pB->get(i).asDouble();
    }
    else
        return false;


    if (Bottle *pB=b.find("play_start_orient").asList())
    {
        playPianoStartOrient[arm].resize(pB->size());

        for (int i=0; i<pB->size(); i++)
            playPianoStartOrient[arm][i]=pB->get(i).asDouble();
    }
    else
        return false;


    if (Bottle *pB=b.find("play_end_pos").asList())
    {
        playPianoEndPos[arm].resize(pB->size());

        for (int i=0; i<pB->size(); i++)
            playPianoEndPos[arm][i]=pB->get(i).asDouble();
    }
    else
        return false;


    if (Bottle *pB=b.find("play_end_orient").asList())
    {
        playPianoEndOrient[arm].resize(pB->size());

        for (int i=0; i<pB->size(); i++)
            playPianoEndOrient[arm][i]=pB->get(i).asDouble();
    }
    else
        return false;
...
void MotorThread::close()
{
    ...
    graspModelPort[LEFT].interrupt();
    graspModelPort[RIGHT].interrupt();
    graspModelPort[LEFT].close();
    graspModelPort[RIGHT].close();
...
bool MotorThread::threadInit()
{
    ...
    graspModelPort[LEFT].open(("/"+name+"/left_arm/grasp_model:o").c_str());
    graspModelPort[RIGHT].open(("/"+name+"/right_arm/grasp_model:o").c_str());
...
bool MotorThread::push(Bottle &options)
{
    ...
    action[arm]->pushAction(xd-3*push_direction*reachSideDisp[arm],reachSideOrient[arm],5.0);
    action[arm]->checkActionsDone(f,true);
    action[arm]->disableContactDetection();
...
bool MotorThread::play(Bottle &options)
{
    ...
    if(!checkOptions(options,"idle"))
    {
        if(checkOptions(options,"index"))
            action[arm]->pushAction("play_piano_index");
        else if(checkOptions(options,"middle"))
            action[arm]->pushAction("play_piano_middle");

        action[arm]->pushAction("play_piano_idle");
    }
    else
        action[arm]->pushAction("play_piano_idle");
...
void MotorThread::extractGraspModel()
{
    for(int idx_arm=0; idx_arm<=1; idx_arm++)
    {    

        if(action[idx_arm]==NULL)
            continue;

        if(calibFingersMutex.check())
        {
            bGraspModel[idx_arm].clear();

            for(int idx_finger=0; idx_finger<5; idx_finger++)
                if (iCub::perception::Node *finger=graspModel[idx_arm]->getNode(fingerName[idx_finger]))
                {
                    Value data; finger->getSensorsData(data);
                    Value out;  finger->getOutput(out);
                    // printf("[Hand %d] %s sensors data = %s; output = %s\n",
                    //        arm,finger->getName().c_str(),data.toString().c_str(),out.toString().c_str());

                    bGraspModel[idx_arm].addDouble(data.asList()->get(0).asList()->get(1).asList()->get(0).asDouble());
                    bGraspModel[idx_arm].addDouble(data.asList()->get(1).asList()->get(1).asList()->get(0).asDouble());
                    bGraspModel[idx_arm].addDouble(data.asList()->get(1).asList()->get(1).asList()->get(1).asDouble());

                    bGraspModel[idx_arm].addDouble(out.asDouble());                 
                }
...
void MotorThread::sendGraspModel()
{
    for(int idx_arm=0; idx_arm<=1; idx_arm++)
        if(bGraspModel[idx_arm].size()>0)
            graspModelPort[idx_arm].write(bGraspModel[idx_arm]);
}



bool MotorThread::startPlay(Bottle &options)
{

    ICartesianControl *ctrl[2];
    Vector oldPos[2],newPos[2];
 
    action[0]->getCartesianIF(ctrl[0]);
    ...
    int context_left;
    int context_right;
    ctrl[0]->storeContext(&context_left);
    ctrl[1]->storeContext(&context_right);

    Vector dof;
    ctrl[0]->getDOF(dof); dof=1.0;
    ctrl[0]->setDOF(dof,dof);

    ctrl[0]->setLimits(0,0.0,0.0);
    ctrl[0]->setLimits(1,0.0,0.0);
    ctrl[0]->setLimits(2,0.0,0.0);

    ctrl[1]->getDOF(dof); dof=1.0;
    ctrl[1]->setDOF(dof,dof);

    ctrl[1]->setLimits(0,0.0,0.0);
    ctrl[1]->setLimits(1,0.0,0.0);
    ctrl[1]->setLimits(2,0.0,0.0);
    ...
    while(Time::now()-currTime<9.0)// && !contact_detected[LEFT] && !contact_detected[RIGHT] )
    {
        ...
        for(int idx_arm=0; idx_arm<=1; idx_arm++)
        {
            ...
            // printf("[Arm: %d] index: %f, middle: %f!\n",idx_arm,bGraspModel[idx_arm].get(1*4+3).asDouble(),bGraspModel[idx_arm].get(2*4+3).asDouble() );

            for(int idx_finger=0; idx_finger<3; idx_finger++)
                if(bGraspModel[idx_arm].get(idx_finger*4+3).asDouble()>thresh_touch[idx_arm])
                {
                    // printf("[Arm: %d] Gotcha!\n",idx_arm );

                    action[idx_arm]->lockActions();
                    action[idx_arm]->syncCheckInterrupt(true);
                    action[idx_arm]->stopControl();
                                
                    action[idx_arm]->unlockActions();
                    action[idx_arm]->syncCheckReinstate();

                    ctrl[idx_arm]->stopControl();

                    contact_detected[idx_arm]=true;

                    action[idx_arm]->pushAction("play_piano_idle");
        ...
        if(unknown_error)
        {
            printf("[Very Helpful Message] !!!!!!!!!!!!!!!!!!!!!!!! UNKNOWN ERROR !!!!!!!!!!!!!!!!!!!!!!!!");
    ...
    if(contact_detected[LEFT] && contact_detected[RIGHT])
        return true;
    else
        return false;
}
...
bool MotorThread::stopPlay(Bottle &options)
{
    int arm=ARM_IN_USE;
